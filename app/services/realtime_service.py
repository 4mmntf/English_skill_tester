...
class RealtimeService:
    ...
    def __init__(self) -> None:
        ...
        self.on_student_transcript: Callable[[str], None] | None = None  # 学生の転写受信時
                    ...
                    self.session.send({
                        ...
                        "session": {
                            ...
                            "turn_detection": {
                                ...
                                "threshold": 0.6,  # 0.0-1.0の範囲、0.6に上げて誤検出を減らす
                                "prefix_padding_ms": 300,  # 発話開始前に含めるパディング
                                "silence_duration_ms": 700  # この時間無音が続いたら発話終了と判断
                            },
                            "temperature": 0.7,
        ...
        def check_audio_buffer():
            ...
            while self.is_connected and self.session:
                try:
                    ...
                    if hasattr(self.session, 'output_audio_buffer') and self.on_audio_received:
    ...
    def _handle_event(self, event: Any) -> None:
        ...
        if event_type == "response.audio.delta":
            ...
            if hasattr(event, 'item') and hasattr(event.item, 'type'):
                if event.item.type == "message" and hasattr(event.item, 'content'):
                    ...
                    for content_item in event.item.content:
                        if hasattr(content_item, 'type') and content_item.type == "audio":
                            # 音声データがある場合
                            if hasattr(content_item, 'audio') and self.on_audio_received:
                                try:
            ...
            # 学生の音声検出開始
            print("学生音声検出開始: conversation.item.input_audio_buffer.speech_started")
        
        elif event_type == "conversation.item.input_audio_buffer.speech_stopped":
            # 学生の音声検出停止
            print("学生音声検出停止: conversation.item.input_audio_buffer.speech_stopped")
